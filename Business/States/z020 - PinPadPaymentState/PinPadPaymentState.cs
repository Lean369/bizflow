...
namespace Business.PinPadPaymentState
{
    public class PinPadPaymentState : StateTransition
    {
        private static readonly NLog.Logger Log = NLog.LogManager.GetLogger("LOG");
        PinPadPaymentStateTableData_Type PinPadPaymentStateTableData; //Tabla con datos provenientes del download.
        PropertiesPinPadPaymentState prop;
        bool ret = false;
        private bool MoreTimeSubscribed = false;
        private List<string> ListOfAck = new List<string>();

        #region "Constructor"
        public PinPadPaymentState(StateTable_Type stateTable)
        {
            this.ActivityName = "PinPadPaymentState";
            this.PinPadPaymentStateTableData = (PinPadPaymentStateTableData_Type)stateTable.Item;
            this.prop = new PropertiesPinPadPaymentState();
            this.prop = this.GetProperties<PropertiesPinPadPaymentState>(out ret, this.prop);
            if (ret)
            {
                if (string.IsNullOrEmpty(this.prop.ScreenNumber))
                    this.prop.ScreenNumber = this.PinPadPaymentStateTableData.ScreenNumber;
                if (string.IsNullOrEmpty(this.prop.NextStateNumber))
                    this.prop.NextStateNumber = this.PinPadPaymentStateTableData.NextStateNumber;
                if (string.IsNullOrEmpty(this.prop.HardwareErrorNextStateNumber))
                    this.prop.HardwareErrorNextStateNumber = this.PinPadPaymentStateTableData.HardwareErrorNextStateNumber;
                if (string.IsNullOrEmpty(this.prop.TimeoutNextStateNumber))
                    this.prop.TimeoutNextStateNumber = this.PinPadPaymentStateTableData.TimeoutNextStateNumber;
                if (string.IsNullOrEmpty(this.prop.CancelNextStateNumber))
                    this.prop.CancelNextStateNumber = this.PinPadPaymentStateTableData.CancelNextStateNumber;
                if (string.IsNullOrEmpty(this.prop.Item1))
                    this.prop.Item1 = this.PinPadPaymentStateTableData.Item1;
                if (string.IsNullOrEmpty(this.prop.Item2))
                    this.prop.Item2 = this.PinPadPaymentStateTableData.Item2;
            }
