...
namespace Business.CheckDepositState
{
    public class CheckDepositState : StateTransition
    {
        private static readonly NLog.Logger Log = NLog.LogManager.GetLogger("LOG");
        CheckDepositStateTableData_Type CheckDepositStateTableData; //Tabla con datos provenientes del download.
        PropertiesCheckDepositState prop;
        bool ret = false;
        private bool MoreTimeSubscribed = false;
        private List<string> ListOfAck = new List<string>();

        #region "Constructor"
        public CheckDepositState(StateTable_Type stateTable)
        {
            this.ActivityName = "CheckDepositState";
            this.CheckDepositStateTableData = (CheckDepositStateTableData_Type)stateTable.Item;
            this.prop = new PropertiesCheckDepositState();
            this.prop = this.GetProperties<PropertiesCheckDepositState>(out ret, this.prop);
            if (ret)
            {
                if (string.IsNullOrEmpty(this.prop.ScreenNumber))
                    this.prop.ScreenNumber = this.CheckDepositStateTableData.ScreenNumber;
                if (string.IsNullOrEmpty(this.prop.NextStateNumber))
                    this.prop.NextStateNumber = this.CheckDepositStateTableData.NextStateNumber;
                if (string.IsNullOrEmpty(this.prop.HardwareErrorNextStateNumber))
                    this.prop.HardwareErrorNextStateNumber = this.CheckDepositStateTableData.HardwareErrorNextStateNumber;
                if (string.IsNullOrEmpty(this.prop.TimeoutNextStateNumber))
                    this.prop.TimeoutNextStateNumber = this.CheckDepositStateTableData.TimeoutNextStateNumber;
                if (string.IsNullOrEmpty(this.prop.CancelNextStateNumber))
                    this.prop.CancelNextStateNumber = this.CheckDepositStateTableData.CancelNextStateNumber;
                if (string.IsNullOrEmpty(this.prop.Item1))
                    this.prop.Item1 = this.CheckDepositStateTableData.Item1;
                if (string.IsNullOrEmpty(this.prop.Item2))
                    this.prop.Item2 = this.CheckDepositStateTableData.Item2;
            }
